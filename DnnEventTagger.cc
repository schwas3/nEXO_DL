...
*/
  ...
  if (xmax - xmin > max_span || ymax - ymin > max_span)
    return true;
*/
  double xmin = -580, ymin = -580;
  // generate data
  const unsigned c_size = 200;
  const unsigned t_size = 255;
  double escale  = 1;//= 91100./tmvaevent->get_charge();
  np::ndarray img = np::zeros(py::make_tuple(c_size, t_size, 3), np::dtype::get_builtin<float>());
  for (unsigned i = 0; i < xposition.size(); i++) {
    if(abs(xposition[i]) > 580)
        continue;//remove channel outside of active LXe
    int h = int((xposition[i] - xmin)/fPitch);
  ...
  for (unsigned i = 0; i < yposition.size(); i++) {
    if(yposition[i] > 580)
        continue;// remove channel outside of active LXe
    int h = int((yposition[i] - ymin)/fPitch);
  ...
  if (0) {
    ...
    char filename[256]; snprintf(filename, sizeof(filename), "images_%d/run_%04d-event_%04d.png", fPitch, 1, 1);
  ...
  if (1) {
    ...
    tmvaevent->set_value(double(res));
